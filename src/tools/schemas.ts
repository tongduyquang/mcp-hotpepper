...
const CommonOptionalParamsSchema = z
  .object({
    // Pagination
    ...
    order: z
      ...
      .optional(), // 1:店名かな順, 2:ジャンルコード順, 3:小エリアコード順, 4:おススメ順(初期値)
...
export const SearchGourmetByKeywordInputSchema =
  CommonOptionalParamsSchema.extend({
    ...
    {
      message:
        'At least one search parameter is required: id, name, name_kana, name_any, tel, address, or keyword',
    },
  );

// Schema for area-based search (SEARCH_GOURMETS_BY_AREA)
export const SearchGourmetsByAreaInputSchema =
  CommonOptionalParamsSchema.extend({
    // Area parameters (at least one required)
    large_service_area: z
      .enum([
        'SS10',
        'SS20',
        'SS30',
    ...
    {
      message:
        'At least one area parameter is required: large_service_area, service_area, large_area, middle_area, or small_area',
    },
  );
  ...
  CommonOptionalParamsSchema.extend({
    ...
    (data) => {
      ...
      return !!(
        ...
        data.special_category_or
      );
    },
    {
      message:
        'At least one recommendation parameter is required: genre, budget, special, special_or, special_category, or special_category_or',
    },
  );

// Type exports for TypeScript usage
export type SearchGourmetByKeywordInput = z.infer<
  typeof SearchGourmetByKeywordInputSchema
>;
export type SearchGourmetsByAreaInput = z.infer<
  typeof SearchGourmetsByAreaInputSchema
>;
export type SearchGourmetsByRecommendInput = z.infer<
  typeof SearchGourmetsByRecommendInputSchema
>;
