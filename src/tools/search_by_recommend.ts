...
export async function handleSearchByRecommend(params: any) {
  try {
    ...
    const searchTerms = [];
    if (validatedParams.genre) searchTerms.push(`genre: ${validatedParams.genre} (${getGenreName(validatedParams.genre)})`);
    if (validatedParams.budget) searchTerms.push(`budget: ${validatedParams.budget} (${getBudgetName(validatedParams.budget)})`);
    if (validatedParams.special) searchTerms.push(`special: ${validatedParams.special}`);
    if (validatedParams.special_or) searchTerms.push(`special_or: ${validatedParams.special_or}`);
    if (validatedParams.special_category) searchTerms.push(`special_category: ${validatedParams.special_category}`);
    if (validatedParams.special_category_or) searchTerms.push(`special_category_or: ${validatedParams.special_category_or}`);
    ...
    if (data.results?.error) {
      const apiError = handleApiError(data.results.error, 'handleSearchByRecommend HotPepper API');
      throw new McpError(ErrorCode.InternalError, apiError.message);
    }

    // Log successful search
    console.log(`Found ${data.results?.results_available || 0} results for recommendation search: ${searchParams}`);
