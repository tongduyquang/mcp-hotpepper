 ...
 * Searches HotPepper Gourmet API by comprehensive keyword search parameters and returns formatted results
...
export async function handleSearchByKeyword(params: any) {
  try {
    ...
    const validatedParams = SearchGourmetByKeywordInputSchema.parse(params);
    ...
    if (validatedParams.keyword)
      searchTerms.push(`keyword: "${validatedParams.keyword}"`);
    if (validatedParams.name)
      searchTerms.push(`name: "${validatedParams.name}"`);
